...
class ClippingPlanesViewer(QtWidgets.QMainWindow):
    ...
    def _create_panel(self):
        ...
        # Display options
        disp = ModernCard("🖼️ Display Options")
        ...
        clear_btn = QtWidgets.QPushButton("🗑️ Clear Planes")
...
def main():
    ...
    # تحديد مسار الفولدر braindataset تلقائياً
    script_dir = os.path.dirname(os.path.abspath(__file__))
    path = os.path.join(script_dir, "braindataset.obj")
    
    print(f"\n📁 Looking for braindataset.obj folder at: {path}")
    
    if not os.path.exists(path):
        print(f"\n❌ Folder 'braindataset.obj' not found at: {path}")
        print("Please make sure the 'braindataset.obj' folder exists in the same directory as this script.\n")
        return
    
    files = sorted(list(set(
        glob.glob(os.path.join(path, "*.obj")) +
        glob.glob(os.path.join(path, "*.OBJ"))
    )))
