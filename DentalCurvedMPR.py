...
class InteractiveCurvedMPR:
    def __init__(self, data, downsample_factor=2):
        ...
        # معالجة خاصة لبيانات الأسنان - تحسين التباين للأنسجة الصلبة
        # استخدام windowing مناسب للعظام والأسنان
        self.data = np.clip(self.data, np.percentile(self.data, 1), np.percentile(self.data, 99.5))
        ...
        self.curve_factor = 0.5  # انحناء أكبر قليلاً للفك السني
        self.active_view = 'axial'  # الـ view النشط
        self.mpr_height = 180  # ارتفاع أكبر للأسنان
        self.mpr_points = 350  # دقة أعلى
    ...
    def setup_ui(self):
        ...
        self.fig.patch.set_facecolor('#1a1a1a')  # خلفية داكنة احترافية
        
        # Grid layout احترافي
        gs = self.fig.add_gridspec(3, 4, height_ratios=[1, 0.05, 2], hspace=0.3, wspace=0.2,
                                   left=0.04, right=0.98, top=0.96, bottom=0.05)
        
        # تحسين colormap للأسنان - استخدام bone أو gray مع تحسينات
        dental_cmap = 'bone'  # أنسب للعظام والأسنان
        
        # Axial view
        self.ax_axial = self.fig.add_subplot(gs[0, 0])
        self.ax_axial.set_facecolor('#000000')
        self.ax_axial.set_title('AXIAL - DENTAL', fontweight='bold', color='white', fontsize=14,
                               pad=10,
        ...
        self.img_axial = self.ax_axial.imshow(self.data[:, :, self.current_slice_ax].T, 
                                              cmap=dental_cmap, origin='lower', picker=True, vmin=0, vmax=1)
        self.ax_axial.axis('off')
        
        # Coronal view
        self.ax_coronal = self.fig.add_subplot(gs[0, 1])
        self.ax_coronal.set_facecolor('#000000')
        self.ax_coronal.set_title('CORONAL - DENTAL', fontweight='bold', color='white', fontsize=14,
                                 pad=10,
        ...
        self.ax_sagittal.set_title('SAGITTAL - DENTAL', fontweight='bold', color='white', fontsize=14,
                                  pad=10,
        ...
        self.info_text_top = self.ax_info_top.text(0.05, 0.5, info_instructions,
                                                   ...
                                                   bbox=dict(boxstyle='round,pad=1', 
                                                           ...
                                                           edgecolor='#FFB300', linewidth=2.5))
        
        # MPR result - مساحة كبيرة
        self.ax_mpr = self.fig.add_subplot(gs[2, :])
        self.ax_mpr.set_facecolor('#000000')
        self.ax_mpr.set_title('⬤ DENTAL PANORAMIC RECONSTRUCTION ⬤', 
                             fontweight='bold', fontsize=17, color='#FFD54F',
                             pad=18,
                             bbox=dict(boxstyle='round,pad=0.8', facecolor='#1a1a1a',
                                     edgecolor='#FFB300', linewidth=3, alpha=0.95))
        self.ax_mpr.text(0.5, 0.5, '🦷 Ready to generate Dental MPR • Define arch path above', 
        ...
        self.btn_undo = Button(ax_undo, '↶ UNDO', 
                              color='#1976D2', hovercolor='#2196F3')
        ...
        self.slider_curve = Slider(ax_curve, 'Arch Curvature', 0.0, 1.0, 
        ...
        # رسم المؤشرات الأولية بتصميم neon
        self.line_axial, = self.ax_axial.plot([], [], '-', color='#FFD54F', linewidth=3, alpha=0.9)
        self.points_axial, = self.ax_axial.plot([], [], 'o', color='#FF6F00', markersize=12, 
                                                markeredgecolor='white', markeredgewidth=3, alpha=1)
        
        self.line_coronal, = self.ax_coronal.plot([], [], '-', color='#FFD54F', linewidth=3, alpha=0.9)
        self.points_coronal, = self.ax_coronal.plot([], [], 'o', color='#FF6F00', markersize=12, 
                                                    markeredgecolor='white', markeredgewidth=3, alpha=1)
        
        self.line_sagittal, = self.ax_sagittal.plot([], [], '-', color='#FFD54F', linewidth=3, alpha=0.9)
        self.points_sagittal, = self.ax_sagittal.plot([], [], 'o', color='#FF6F00', markersize=12, 
                                                      markeredgecolor='white', markeredgewidth=3, alpha=1)
        ...
        self.info_text = self.fig.text(0.5, 0.985, self.get_info_text(), 
                                       ...
                                       bbox=dict(boxstyle='round,pad=0.7', facecolor='#263238', 
                                               alpha=0.95, edgecolor='#FFB300', linewidth=2.5))
    ...
    def get_info_text(self):
        ...
        return (f"🦷 Selected Points: {len(self.points)} | "
                f"Arch Curvature: {self.curve_factor:.2f} | "
                f"Resolution: {self.mpr_points}×{self.mpr_height} px ●")
    ...
    def create_curve_high_res(self):
        ...
        try:
            tck, u = splprep([control_points[:, 0], control_points[:, 1], control_points[:, 2]], 
                            s=0, k=min(3, len(control_points)-1))
            u_new = np.linspace(0, 1, self.mpr_points)  # دقة قابلة للتعديل
            curve = np.array(splev(u_new, tck)).T
            return curve
        except:
            return points_arr
    ...
    def generate_mpr(self, event):
        ...
        if len(self.points) < 2:
            ...
            self.ax_mpr.clear()
            self.ax_mpr.text(0.5, 0.5, '⚠ Add at least 2 points to define dental arch!', 
        ...
        print("\n" + "="*50)
        print("Generating Dental Panoramic MPR...")
        ...
        # تحسين التباين خاص للأسنان (bone/enamel enhancement)
        if mpr_image.max() > 0:
            # استخدام windowing خاص بالعظام والأسنان
            p2, p98 = np.percentile(mpr_image[mpr_image > 0], (2, 98))
            mpr_enhanced = np.clip((mpr_image - p2) / (p98 - p2 + 1e-10), 0, 1)
        ...
        # استخدام colormap مناسب للأسنان
        im = self.ax_mpr.imshow(mpr_enhanced, cmap='bone', 
        ...
        cbar.set_label('DENSITY', fontsize=11, fontweight='bold', color='white')
        cbar.ax.tick_params(labelsize=9, colors='white')
        cbar.outline.set_edgecolor('#FFB300')
        cbar.outline.set_linewidth(2)
        
        self.ax_mpr.set_title(f'🦷 DENTAL PANORAMIC MPR  |  {time.time()-start_time:.2f}s  |  High Resolution ⬤', 
                             fontweight='bold', fontsize=16, color='#FFD54F',
                             pad=15,
                             bbox=dict(boxstyle='round,pad=0.8', facecolor='#1a1a1a',
                                     edgecolor='#FFB300', linewidth=3, alpha=0.98))
        self.ax_mpr.set_xlabel('Position along dental arch  →', fontsize=13, fontweight='bold', color='#64B5F6')
        self.ax_mpr.set_ylabel('⟂  Perpendicular distance', fontsize=13, fontweight='bold', color='#64B5F6')
        self.ax_mpr.grid(True, alpha=0.15, linestyle='--', linewidth=1, color='#FFB300')
        
        # تحسين ticks
        self.ax_mpr.tick_params(labelsize=10, colors='#90A4AE')
        self.ax_mpr.spines['bottom'].set_color('#FFB300')
        self.ax_mpr.spines['left'].set_color('#FFB300')
        ...
        info_str = (f"📊 Resolution: {mpr_enhanced.shape[1]}×{mpr_enhanced.shape[0]} px  |  "
                   f"📏 Arch Length: {path_length:.1f} voxels  |  "
        ...
        self.fig.canvas.draw_idle()
        print(f"✓ Dental MPR generated in {time.time()-start_time:.1f} seconds")
    ...
    def clear_points(self, event):
        ...
        self.points = []
        self.ax_mpr.clear()
        self.ax_mpr.set_title('🦷 DENTAL PANORAMIC MPR - Define arch path and click "Generate MPR" ★', 
                             fontweight='bold', fontsize=13, color='#FFD54F')
        self.ax_mpr.axis('off')
        self.update_display()
...
def main():
    """البرنامج الرئيسي"""
    print("="*60)
    print("🦷 INTERACTIVE DENTAL CURVED MPR VIEWER 🦷")
    print("="*60)
    
    # تحميل البيانات
    print("\nLoading dental data...")
    file_path = "Dental.nii"
    ...
    try:
        ...
        print("INSTRUCTIONS:")
        print("─"*60)
        print("  1. 🖱️  Click on any view to add points along dental arch")
        print("  2. 📍 Add at least 2 points (3-5 recommended for best results)")
        print("  3. 🎚️  Adjust 'Arch Curvature' slider to match dental anatomy")
        print("  4. ▶️  Click 'Generate MPR' to create panoramic view")
        print("  5. ↶  Use 'Undo Last' to remove last point")
        print("  6. ✖️  Use 'Clear Points' to start over")
        print("\n💡 TIP: For dental arches, place points following the jaw curve")
        print("="*60 + "\n")
        ...
        print("Please make sure 'Dental.nii' is in the same directory as this script.")
